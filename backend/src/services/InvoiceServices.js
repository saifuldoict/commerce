...
const CreateInvoiceService = async (req) => {
    ...
    const existUser = await UserModel.findOne({_id:user_id}).lean()
    if(!existUser?._id){
...
// =============Step 01: Calculate Total Payable & Vat=====================================================================================
    ...
    let matchStage={$match:{userID:existUser?._id}}
...
// const profileObj =await ProfileModel.findOne({}).lean()
// console.log(profileObj)
    let Profile=await ProfileModel.aggregate([matchStage]);
    if(!Profile.length){
        return {success:false,message:"User not founds!"}
    }
